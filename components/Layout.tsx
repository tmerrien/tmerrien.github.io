...
const SilkBackground = dynamic(
  () => import('./ui/silk-background').then((m) => ({ default: m.SilkBackground })),
  { ssr: false }
);
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen md:h-screen relative">
      {/* Silk background — desktop only (saves CPU on mobile) */}
      {isDesktop && <SilkBackground />}

      {/* Mobile header */}
      <header className="md:hidden fixed top-0 w-full z-50 bg-[var(--bg)]/80 backdrop-blur-xl border-b border-white/20">
        <div className="flex items-center justify-center px-5 h-12">
          <span className="font-['Space_Grotesk'] font-bold text-sm tracking-tighter text-[var(--on-surface)]">
            TM
          </span>
        </div>
      </header>
      ...
      {/* Footer — mobile only */}
      <footer className="md:hidden py-8 px-5">
