...
export default function Projects() {
  return (
    <section id="projects" className="py-8 sm:py-12 px-5 sm:px-6 max-w-4xl mx-auto">
      ...
      <FadeIn>
        <h2 className="text-xs font-mono gradient-text font-bold mb-6 tracking-wide">PROJECTS</h2>
      </FadeIn>
      <FadeInStagger className="grid sm:grid-cols-2 gap-6">
        {projects.map((project) => (
          <FadeInItem key={project.title}>
            <a href={project.url} target="_blank" rel="noopener noreferrer" className="group cursor-pointer block h-full">
              <Card className="h-full card-glow hover:border-[#2563EB]/40 dark:hover:border-[#60A5FA]/40 transition-all duration-200">
                ...
                <CardContent>
                  <h3 className="text-base font-semibold mb-2 font-mono">{project.title}</h3>
                  <p className="text-sm text-muted-foreground leading-relaxed mb-4">{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((t) => (
                      <Badge key={t} variant="outline" className="text-[11px] font-mono font-normal">{t}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
