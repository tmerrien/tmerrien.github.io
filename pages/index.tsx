        ...
        >
          ...
          >
            ...
            <div className="relative">
              ...
              <h1 className="font-['Space_Grotesk'] text-4xl font-bold tracking-tighter">Tanguy Merrien</h1>
              <p className="text-[var(--secondary)] text-lg mt-1">AI &amp; Software Engineer</p>
            </div>
            <div className="relative mt-auto pt-4 flex items-center gap-4">
              <div className="w-11 h-11 rounded-full glass-inner-border overflow-hidden">
                <img src="/images/avatar.jpg" alt="" className="w-full h-full object-cover" />
              </div>
              <div>
                <p className="text-xs font-['Space_Grotesk'] font-bold text-[var(--on-surface-variant)] uppercase tracking-wider">Status</p>
            ...
            <div className="relative p-7 h-full flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-['Space_Grotesk'] text-xs uppercase tracking-widest text-[var(--primary)] font-bold">Featured Research</span>
                </div>
                <h2 className="font-['Space_Grotesk'] text-4xl font-extrabold tracking-tighter leading-tight">WoZ Platform for LLM Evaluation</h2>
                <p className="text-[var(--secondary)] mt-4 text-base leading-relaxed max-w-sm">
              ...
              <div className="mt-4">
                <Button variant="solid" size="lg" className="!rounded-full !bg-[var(--primary)] !text-[var(--on-primary)] !border-transparent hover:!opacity-90">
                  <a href="https://github.com/tmerrien/oz-upei" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-['Space_Grotesk'] text-base font-bold">
                    View Thesis <span className="material-symbols-outlined text-base">arrow_outward</span>
                  </a>
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-5">
              {jobs.map((job) => (
                <div key={job.num} className="flex gap-3">
                  <div className="text-[var(--primary)] font-['Space_Grotesk'] font-bold text-lg">{job.num}</div>
                  <div>
                    <h4 className="font-['Space_Grotesk'] font-bold text-base tracking-tight leading-tight">{job.company}</h4>
                    <p className="text-xs text-[var(--secondary)]">{job.role} &middot; {job.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            ...
            <div className="space-y-4">
              {Object.entries(skills).map(([category, items]) => (
                <div key={category}>
                  <p className="text-[11px] font-['Space_Grotesk'] font-bold text-[var(--primary)] mb-2 uppercase tracking-widest">{category}</p>
          ...
          >
            <div>
              ...
              <p className="text-base text-[var(--secondary)] mt-1">B.Sc. Honours in Computer Science</p>
              <p className="text-sm text-[var(--outline)] mt-0.5">Minor: Physics</p>
            </div>
            <div>
              <div className="flex items-center gap-2 pt-3 border-t border-white/20">
                <span className="material-symbols-outlined text-[var(--primary)] text-xl">school</span>
                <span className="font-['Space_Grotesk'] text-xs font-medium text-[var(--on-surface-variant)] uppercase tracking-widest">Honours Distinction</span>
              </div>
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/20">
                <span className="material-symbols-outlined text-[var(--primary)] text-xl">music_note</span>
                <div>
                  <p className="font-['Space_Grotesk'] text-xs font-medium text-[var(--on-surface-variant)] uppercase tracking-widest">Harris Institute</p>
                  <p className="text-xs text-[var(--secondary)]">Sound Engineering</p>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar">
              {projects.map((project) => (
                <a key={project.title} href={project.url} target="_blank" rel="noopener noreferrer"
                  className="flex-shrink-0 w-44 glass-inner-border bg-white/30 rounded-xl p-4 hover:bg-white/50 transition-colors group"
                >
                  <span className="font-['Space_Grotesk'] text-[10px] uppercase tracking-widest text-[var(--primary)] font-bold">{project.tag}</span>
                  <h4 className="font-['Space_Grotesk'] font-bold text-sm mt-1.5 leading-snug">{project.title}</h4>
                  <p className="text-[11px] text-[var(--secondary)] mt-1 leading-relaxed line-clamp-2">{project.description}</p>
                  <div className="flex items-center gap-1 mt-2">
                    <span className="text-[11px] text-[var(--outline)] group-hover:text-[var(--primary)] transition-colors">View</span>
                    <span className="material-symbols-outlined text-sm text-[var(--outline)] group-hover:text-[var(--primary)] transition-all">arrow_outward</span>
                  </div>
                </a>
              ))}
            </div>
          ...
          >
            <a href="mailto:tmerrien@outlook.com" className="flex items-center gap-4 w-full">
              ...
              <div>
                <h4 className="font-['Space_Grotesk'] font-bold text-base">Get in Touch</h4>
                <p className="text-sm text-[var(--secondary)]">AI/agent engineering &mdash; remote from Ottawa</p>
              </div>
            </a>
            <div className="flex gap-4 flex-shrink-0 ml-3">
